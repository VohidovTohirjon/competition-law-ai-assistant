...
- **The general mode never answers law from memory.** Mode is a contract: *general* never touches retrieval. But a legal question asked there is not answered from the model's parametric memory either — left to itself the model invented a law title, an article number and a fine range. A legal-intent question in general mode gets a concept-level explanation with no statute names, article numbers or figures, a visible "not based on the statute base" notice, and a pointer to the legal mode. If the model states a legal specific anyway, a deterministic check replaces the whole answer with that pointer.
